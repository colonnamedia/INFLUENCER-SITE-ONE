...
const CAT_COLOR = { Business: 'var(--c-blue)', Weddings: 'var(--c-pink)', Events: 'var(--c-orange)' }
    ...
    <>
      ...
      <header className="hero-full">
        <div className="hero-full__bg">
          <img src={CREATOR.heroPhoto} alt="" />
        </div>
        <div className="hero-full__scrim" />
        <div className="wrap hero-full__inner">
          <span className="hero-full__pill">Now booking · 2026</span>
          <p className="eyebrow eyebrow--light">{CREATOR.role} · {CREATOR.location}</p>
          <h1 className="hero-full__title">
            {CREATOR.name.split(' ')[0]} <em>{CREATOR.name.split(' ').slice(1).join(' ')}</em>
          </h1>
          <p className="hero-full__sub">{CREATOR.tagline} One local team for the photo and video your business, your wedding, or your event deserves.</p>
          <div className="hero-full__cta">
            <a href="#work" className="btn btn--white">See our work <ArrowUpRight /></a>
            <Link to="/book" className="btn btn--outline-white">Book a shoot</Link>
          </div>
        </div>
        <a href="#work" className="hero-full__scroll">Scroll ↓</a>
      </header>
            ...
            {work.map((p) => (
              <div className="work" key={p.id}>
                ...
                <div className="work__overlay">
                  <span className="work__cat" style={{ background: CAT_COLOR[p.category] }}>{p.category}</span>
